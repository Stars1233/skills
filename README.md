...
| `train-cross-encoder` | Train or fine-tune CrossEncoder reranker models that score (query, passage) pairs jointly to re-rank first-stage retrieval results. Covers pointwise, pairwise, and listwise ranking losses, distillation, hard-negative mining, evaluators, LoRA, and Hugging Face Hub publishing. | [SKILL.md](skills/train-cross-encoder/SKILL.md) |
| `train-sentence-transformer` | Train or fine-tune SentenceTransformer (bi-encoder) embedding models for semantic similarity, retrieval, clustering, and classification. Covers loss selection, hard-negative mining, NanoBEIR / STS evaluators, multi-task training, distillation, LoRA, Matryoshka, and Hugging Face Hub publishing. | [SKILL.md](skills/train-sentence-transformer/SKILL.md) |
| `train-sparse-encoder` | Train or fine-tune SparseEncoder (SPLADE) models that produce interpretable sparse embeddings compatible with inverted-index backends. Covers SPLADE pooling with FLOPS regularization, sparse losses, hard-negative mining, sparsity-tracking evaluators, distillation, and Hugging Face Hub publishing. | [SKILL.md](skills/train-sparse-encoder/SKILL.md) |
